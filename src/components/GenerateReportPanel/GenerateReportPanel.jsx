...
export default function GenerateReportPanel({ weekStart, onGenerate }) {
  ...
  const [status, setStatus] = useState(null); // { text, type, reportUrl, sharedCopyUrl }
  ...
  async function handleSubmit(event) {
    ...
    if (result.success) {
      setStatus({
        ...
        sharedCopyUrl: result.sharedCopyUrl,
      });
    ...
    <form className="generate-report" onSubmit={handleSubmit}>
      ...
      {status?.text && (
        <p className={`generate-report-message ${status.type}`}>
          ...
          {status.sharedCopyUrl && (
            <>
              {' · '}
              <a href={status.sharedCopyUrl} target="_blank" rel="noreferrer">
                Open shared copy
              </a>
            </>
          )}
